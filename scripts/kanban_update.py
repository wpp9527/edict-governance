...
_CANONICAL_DATA_DIR = pathlib.Path(os.environ['EDICT_TASK_DATA_DIR']).expanduser() if os.environ.get('EDICT_TASK_DATA_DIR') else (pathlib.Path.home() / '.openclaw' / 'workspace-main' / 'data')
_FALLBACK_DATA_DIR = _BASE / 'data'
_ACTIVE_DATA_DIR = _CANONICAL_DATA_DIR if _CANONICAL_DATA_DIR.is_dir() else _FALLBACK_DATA_DIR
TASKS_FILE = _ACTIVE_DATA_DIR / 'tasks_source.json'
REFRESH_SCRIPT = _ACTIVE_DATA_DIR.parent / 'scripts' / 'refresh_live_data.py'
...
_REFRESH_SIGNAL_FILE = _ACTIVE_DATA_DIR / '.refresh_pending'
...
# ── 审计日志 ──
AUDIT_FILE = _ACTIVE_DATA_DIR / 'audit_log.json'
...
def _iter_task_data_dirs():
    """Yield likely task data directories in priority order.

    关键修复：优先固定 canonical 数据源，只把其他 workspace 作为只读兜底搜索。
    """
    seen = set()
    candidates = []
    ...
    base_dirs = [
        _ACTIVE_DATA_DIR,
        _FALLBACK_DATA_DIR,
        pathlib.Path.home() / '.openclaw' / 'workspace-taizi' / 'data',
    ]
    for d in base_dirs:
    ...
    home_root = pathlib.Path.home() / '.openclaw'
    if home_root.exists():
        for ws in sorted(home_root.glob('workspace-*/data')):
            if ws not in seen and ws.is_dir():
                seen.add(ws)
...
def _find_task_across_dirs(task_id):
    """跨数据目录查找任务，返回 (task, tasks, tasks_file) 或 (None, None, None)。"""
    for d in _iter_task_data_dirs():
        tf = d / 'tasks_source.json'
        if not tf.exists():
            continue
        try:
            tasks = atomic_json_read(tf, [])
        except Exception:
            continue
...
def cmd_flow(task_id, from_dept, to_dept, remark):
    ...
    _, _, task_file = _find_task_across_dirs(task_id)
    task_file = task_file or TASKS_FILE
...
def cmd_done(task_id, output_path='', summary=''):
    ...
    def modifier(tasks):
        ...
        if not t:
            ...
            return tasks
        t['state'] = 'Done'
        # 允许 output_path 为空时，仍保留可读的完成摘要，避免 Done 但输出为空
        t['output'] = output_path or summary or '任务已完成'
        t['outputSummary'] = summary or '任务已完成'
        ...
        if output_path:
            ...
            t['outputMeta'] = {"exists": False, "lastModified": None}
        # 关键修复：禁用 scheduler 并更新 lastProgressAt，防止调度器继续认为任务停滞
    ...
    _append_audit(task_id, _infer_agent_id_from_runtime(), 'done', None, output_path or summary, summary)
...
def validate_task_consistency(task):
    """轻量一致性检查，返回问题列表（不阻断主流程）。"""
    issues = []
    state = task.get('state')
    org = task.get('org')
    flow = task.get('flow_log') or []
    if state == 'Done' and not (task.get('output') or task.get('outputSummary')):
        issues.append('Done 但 output 为空')
    if state == 'Taizi' and flow:
        last_to = flow[-1].get('to')
        if last_to in ('中书省', '门下省', '尚书省', '六部'):
            issues.append(f'state=Taizi 但最后流转已到 {last_to}')
    if state in ('Doing', 'Review', 'Assigned', 'Next') and org == '太子':
        issues.append('执行态但 org 仍为太子')
    return issues
...
MEMORY_DIR = _ACTIVE_DATA_DIR / 'agent_memory'
TASK_MEMORY_DIR = _ACTIVE_DATA_DIR / 'task_memory'
SHARED_MEMORY_FILE = _ACTIVE_DATA_DIR / 'shared_memory.json'

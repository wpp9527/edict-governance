...
def test_flow_finds_task_across_data_dirs(tmp_path, monkeypatch):
    ...
    (other_data / 'tasks_source.json').write_text(json.dumps([
        {'id': 'T-X', 'title': 'cross-dir flow test', 'state': 'Zhongshu', 'flow_log': []}
    ]))
    ...
    tasks = json.loads((other_data / 'tasks_source.json').read_text())
    assert tasks[0]['org'] == '中书省'
    assert tasks[0]['flow_log'][-1]['to'] == '中书省'

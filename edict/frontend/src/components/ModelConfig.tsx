...
import { OFFICIAL_ROLE_LABEL_MAP, OFFICIAL_ROLE_EMOJI_MAP, OFFICIAL_ROLE_ROLE_MAP } from '../config/generated/officialUi';
...
export default function ModelConfig() {
  ...
  const resetMC = (agentId: string) => {
    const ag = semanticAgents.find((a) => a.id === agentId);
          ...
          return (
            <div className="mc-card" key={ag.id}>
              <div className="mc-top">
                <span className="mc-emoji">{OFFICIAL_ROLE_EMOJI_MAP[ag.id] || ag.emoji || '🏛️'}</span>
                <div>
                  <div className="mc-name">
                    {(OFFICIAL_ROLE_LABEL_MAP[ag.id] || ag.label)}{' '}
                    <span style={{ fontSize: 11, color: 'var(--muted)' }}>{ag.id}</span>
                  </div>
                  <div className="mc-role">{OFFICIAL_ROLE_ROLE_MAP[ag.id] || ag.role}</div>
                </div>
              </div>

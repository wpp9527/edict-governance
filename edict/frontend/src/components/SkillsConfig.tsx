...
import { OFFICIAL_ROLE_ID_SET } from '../config/generated/officialRoles';
import { OFFICIAL_ROLE_LABEL_MAP, OFFICIAL_ROLE_EMOJI_MAP } from '../config/generated/officialUi';
...
export default function SkillsConfig() {
  ...
  const semanticAgents = (agentConfig.agents || []).filter((ag) => OFFICIAL_ROLE_ID_SET.has(ag.id));

  // ── 本地技能面板 ──
  const localPanel = (
    <div>
      <div className="skills-grid">
        {semanticAgents.map((ag) => (
          <div className="sk-card" key={ag.id}>
            <div className="sk-hdr">
              <span className="sk-emoji">{OFFICIAL_ROLE_EMOJI_MAP[ag.id] || ag.emoji || '🏛️'}</span>
              <span className="sk-name">{OFFICIAL_ROLE_LABEL_MAP[ag.id] || ag.label}</span>
          ...
          <div style={{ marginTop: 14, background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 12, padding: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
              ...
              >
                ...
                {semanticAgents.map((ag) => (
                  <option key={ag.id} value={ag.id}>{ag.emoji} {OFFICIAL_ROLE_LABEL_MAP[ag.id] || ag.label} ({ag.id})</option>
                ))}
              </select>
            </div>
      ...
      {remoteLoading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {remoteSkills.map((sk) => {
            ...
            const agInfo = semanticAgents.find((a) => a.id === sk.agentId);
            return (
              <div
                key={key}
                style={{
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
                    ...
                    <span style={{ fontSize: 11, color: 'var(--muted)', background: 'var(--panel2)', padding: '2px 8px', borderRadius: 6 }}>
                      {(agInfo && OFFICIAL_ROLE_EMOJI_MAP[agInfo.id]) || agInfo?.emoji} {(agInfo && OFFICIAL_ROLE_LABEL_MAP[agInfo.id]) || agInfo?.label || sk.agentId}
                    </span>
                  </div>
                  {sk.description && (
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '1px solid var(--line)', paddingBottom: 0 }}>
        {[
          { key: 'local', label: '🏛️ 本地技能', count: semanticAgents.reduce((n, a) => n + (a.skills?.length || 0), 0) },
            ...
            <div className="modal-body">
              ...
              <form onSubmit={submitAddRemote} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div>
                  ...
                  >
                    <option value="">— 选择 Agent —</option>
                    {(semanticAgents || agentConfig.agents).map((ag) => (
                      <option key={ag.id} value={ag.id}>{ag.emoji} {OFFICIAL_ROLE_LABEL_MAP[ag.id] || ag.label} ({ag.id})</option>
                    ))}
                  </select>
                </div>

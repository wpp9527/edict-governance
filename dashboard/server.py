...
DATA = BASE.parent / "data"  # 原始数据目录（保留兼容）

def get_config_data_dir():
    """获取配置数据目录（与任务数据统一）"""
    return get_task_data_dir()
SCRIPTS = BASE.parent / 'scripts'
CANONICAL_TASK_DATA_DIR = OCLAW_HOME / 'workspace-main' / 'data'
...
def _iter_task_data_dirs():
    ...
    优先级：
    1. EDICT_TASK_DATA_DIR 显式指定
    2. ~/.openclaw/workspace-main/data（canonical）
    3. 项目本地 data（兼容旧部署）
    4. 其他 workspace-*/data（仅兜底，不再按“最新/最多”猜测）
    """
    dirs = []
    explicit = os.environ.get('EDICT_TASK_DATA_DIR', '').strip()
    if explicit:
        dirs.append(pathlib.Path(explicit).expanduser())
    dirs.append(CANONICAL_TASK_DATA_DIR)
    dirs.append(DATA)
...
def get_task_data_dir():
    ...
    关键修复：不再根据“任务数/更新时间”动态猜测，避免前台读 B、脚本写 A。
    """
    ...
    for d in _iter_task_data_dirs():
        ...
        if tf.exists():
            _ACTIVE_TASK_DATA_DIR = d
            log.info(f'任务数据源: {_ACTIVE_TASK_DATA_DIR}')
            return _ACTIVE_TASK_DATA_DIR
    _ACTIVE_TASK_DATA_DIR = CANONICAL_TASK_DATA_DIR if CANONICAL_TASK_DATA_DIR.is_dir() else DATA
    log.warning(f'任务数据源回退到: {_ACTIVE_TASK_DATA_DIR}')
    return _ACTIVE_TASK_DATA_DIR
...
def read_skill_content(agent_id, skill_name):
    ...
    cfg = read_json(get_config_data_dir() / 'agent_config.json', {})
...
def migrate_notification_config():
    ...
    cfg_path = get_config_data_dir() / 'morning_brief_config.json'
...
def push_notification():
    """通用消息推送 (支持多渠道)"""
    cfg = read_json(get_config_data_dir() / 'morning_brief_config.json', {})
    ...
    if not channel_cls.validate_webhook(webhook):
        ...
        return
    brief = read_json(get_config_data_dir() / 'morning_brief.json', {})
...
_AGENT_DEPTS = [
 ...
 {'id':'zaochao', 'label':'钦天监', 'emoji':'🛰️', 'role':'早朝官', 'rank':'正三品'},

 {'id':'duzhisi', 'label':'度支司', 'emoji':'🪙', 'role':'度支使', 'rank':'从二品'},

 {'id':'taixuesi', 'label':'太学寺', 'emoji':'📚', 'role':'太学祭酒', 'rank':'从二品'},

 {'id':'jinyiwei', 'label':'锦衣卫', 'emoji':'🛡️', 'role':'锦衣卫指挥使', 'rank':'正三品'},

 {'id':'qingbaoshi', 'label':'情报使', 'emoji':'📡', 'role':'情报使', 'rank':'从三品'},

 {'id':'jinengdaoshi', 'label':'技能导师', 'emoji':'🧭', 'role':'技能导师', 'rank':'从三品'},
...
def handle_scheduler_scan(threshold_sec=600):
    ...
    for task in tasks:
        ...
        # 关键修复：检查 flow_log 是否已显示任务完成（工部/执行部门 → 皇上）
        flow = task.get('flow_log', [])
        if flow:
            last_flow = flow[-1]
            if last_flow.get('to') == '皇上' or '已完成' in last_flow.get('remark', ''):
                # 任务已回奏，强制同步状态并跳过调度
                if state != 'Done':
                    task['state'] = 'Done'
                    task['org'] = '回奏'
                    task['now'] = last_flow.get('remark', '任务已完成')
                    changed = True
                continue
...
def dispatch_for_state(task_id, task, new_state, trigger='state-transition'):
    ...
    _msgs = {
        'taizi': (
            ...
            f'⚠️ 看板已有此任务，请勿重复创建。\n'
            f'请立刻执行：\n'
            f'1) 用 kanban_update.py progress 记录当前进展\n'
            f'2) 用 kanban_update.py flow 写入“太子 → 中书省”\n'
            f'3) 用 kanban_update.py state 将状态同步推进到 Zhongshu\n'
            f'注意：只写 flow 不够，必须同步 state。'
        ),
        'zhongshu': (
            f'📜 旨意已到中书省，请起草方案\n'
            f'任务ID: {task_id}\n'
    ...
    def _do_dispatch():
        try:
            ...
            _agent_cfg = read_json(get_config_data_dir() / 'agent_config.json', {})
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p in ('', '/dashboard', '/dashboard.html'):
            ...
            self.send_json(_align_agent_config_response(read_json(get_config_data_dir() / 'agent_config.json', {})))
        elif p == '/api/officials-stats':
            self.send_json(_align_officials_stats_response(read_json(get_config_data_dir() / 'officials_stats.json', {}), read_json(get_config_data_dir() / 'agent_config.json', {})))
    ...
    def do_POST(self):
        ...
        if p == '/api/set-model':
            ...
            pending_path = get_config_data_dir() / 'pending_model_changes.json'
        ...
        # Fix #139: 设置派发渠道（feishu/telegram/wecom/signal/tui）
            ...
            def _set_channel(cfg):
                ...
                return cfg
            atomic_json_update(get_config_data_dir() / 'agent_config.json', _set_channel, {})
...
# ===== Stage17 backend API alignment helpers =====
_EXTENDED_OFFICIAL_ROLE_FILE = pathlib.Path(__file__).resolve().parent / 'official_roles.extended.json'

def _load_extended_official_roles():
    data = read_json(_EXTENDED_OFFICIAL_ROLE_FILE, [])
    return data if isinstance(data, list) else []

def _default_model_from_agent_config(agent_cfg):
    ...
    preferred = (
        ...
        'hubu', 'libu', 'bingbu', 'xingbu', 'gongbu', 'libu_hr', 'zaochao'
    )
    for pid in preferred:
        for a in agents:
            if isinstance(a, dict) and a.get('id') == pid and a.get('model'):
                return a.get('model')
    for a in agents:
...
def _align_agent_config_response(payload):
    payload = payload if isinstance(payload, dict) else {}
    agents = payload.get('agents', [])
    agents = agents if isinstance(agents, list) else []
    seen = {a.get('id') for a in agents if isinstance(a, dict)}
    default_model = _default_model_from_agent_config(payload)
    for role in _load_extended_official_roles():
        rid = role.get('id')
        # 跳过 main（OpenClaw 运行时默认入口，不属于正式官职）
        if not rid or rid in seen or rid == 'main':
            continue
        agents.append({
            'id': rid,
            'label': role.get('label', rid),
            'emoji': role.get('emoji', '🏛️'),
            'role': role.get('role', role.get('label', rid)),
            'model': default_model,
            'skills': [],
        })
        seen.add(rid)
...
def _align_officials_stats_response(payload, agent_cfg):
    payload = payload if isinstance(payload, dict) else {}
    officials = payload.get('officials', [])
    officials = officials if isinstance(officials, list) else []

    aligned_agent_cfg = _align_agent_config_response(agent_cfg if isinstance(agent_cfg, dict) else {})
    agent_model_by_id = {}
    for a in aligned_agent_cfg.get('agents', []):
        if isinstance(a, dict) and a.get('id'):
            agent_model_by_id[a.get('id')] = a.get('model', '') or ''

    default_model = _default_model_from_agent_config(aligned_agent_cfg)
    by_id = {o.get('id'): o for o in officials if isinstance(o, dict) and o.get('id')}

    for role in _load_extended_official_roles():
        rid = role.get('id')
        # 跳过 main（OpenClaw 运行时默认入口，不属于正式官职）
        if not rid or rid in by_id or rid == 'main':
            continue
        model = agent_model_by_id.get(rid) or default_model
    ...
    officials.sort(key=lambda x: (-_safe_num(x.get('merit_score')), x.get('label', ''), x.get('id', '')))
    for idx, item in enumerate(officials, 1):
        item['merit_rank'] = idx

    payload['officials'] = officials
    if 'totals' not in payload or not isinstance(payload.get('totals'), dict):
        payload['totals'] = {'tasks_done': 0, 'cost_cny': 0}
    if 'top_official' not in payload:
        payload['top_official'] = officials[0].get('label', '') if officials else ''
    return payload
# ===== /Stage17 backend API alignment helpers =====
